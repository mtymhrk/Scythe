...
test_quasiquote_internal(ScmObj expected, ScmObj template, ScmObj values)
{
  ...
  qq = scm_compile_qq_template(template);
  TEST_ASSERT(scm_qqtmpl_p(qq));
  TEST_ASSERT_SCM_EQUAL(expected, scm_substitute_qq_template(qq, values));
}
...
test_quasiquote_compile_error(const char *template)
{
  ...
  TEST_ASSERT_SCM_NULL(scm_compile_qq_template(t));
}
...
test_quasiquote_substitute_error_internal(ScmObj template, ScmObj values)
{
  ...
  qq = scm_compile_qq_template(template);
  TEST_ASSERT(scm_qqtmpl_p(qq));
  TEST_ASSERT_SCM_NULL(scm_substitute_qq_template(qq, values));
}
...
test_quasiquote_unquoted_expr(const char *expected, const char *template)
{
  ...
  TEST_ASSERT_EQUAL_INT(scm_vector_length(e),
                        scm_qqtmpl_nr_unquoted_expr(qq));

  n = scm_vector_length(e);
  for (size_t i = 0; i < n; i++) {
    TEST_ASSERT_SCM_EQUAL(scm_vector_ref(e, i),
                          scm_qqtmpl_unquoted_expr(qq, i));
  }
}
...
test_quasiquote_original_template(const char *template)
{
  ...
  TEST_ASSERT_SCM_EQUAL(t, scm_qqtmpl_template(qq));
}

TEST(compiler, quasiquote__literal)
{
  test_quasiquote("a", "a", "()");
}

TEST(compiler, quasiquote__literal__list)
{
  test_quasiquote("(a b c)", "(a b c)", "()");
}

TEST(compiler, quasiquote__literal__improper_list)
{
  test_quasiquote("(a b . c)", "(a b . c)", "()");
}

TEST(compiler, quasiquote__literal__vector)
{
  test_quasiquote("#(a b c)", "#(a b c)", "()");
}

TEST(compiler, quasiquote__unquote)
{
  test_quasiquote("a", ",expr", "(a)");
}

TEST(compiler, quasiquote__unquote__list)
{
  test_quasiquote("(a b c)", "(a ,expr c)", "(b)");
}

TEST(compiler, quasiquote__unquote__list_2)
{
  test_quasiquote("(a b c)", "(a ,expr ,expr)", "(b c)");
}

TEST(compiler, quasiquote__unquote__improper_list)
{
  test_quasiquote("(a b . c)", "(a b . ,expr)", "(c)");
}

TEST(compiler, quasiquote__unquote__vector)
{
  test_quasiquote("#(a b c)", "#(a ,expr c)", "(b)");
}

TEST(compiler, quasiquote__unquote__vector_2)
{
  test_quasiquote("#(a b c)", "#(a ,expr ,expr)", "(b c)");
}

TEST(compiler, quasiquote__unquote_splicing__list)
{
  test_quasiquote("(a b c d)", "(a ,@expr d)", "((b c))");
}

TEST(compiler, quasiquote__unquote_splicing__vector)
{
  test_quasiquote("#(a b c d)", "#(a ,@expr d)", "((b c))");
}

TEST(compiler, quasiquote__nest)
{
...
TEST(compiler, quasiquote__invalid_unquote_splicing__ERROR)
{
  test_quasiquote_compile_error(",@expr");
}

TEST(compiler, quasiquote__invalid_unquote_splicing_list__ERROR)
{
  test_quasiquote_compile_error("(a b . ,@expr)");
}

TEST(compiler, quasiquote__too_few_values__ERROR)
{
  test_quasiquote_substitute_error("(a ,expr ,expr)", "(b)");
}

TEST(compiler, quasiquote_original_template)
{
  test_quasiquote_original_template("(a ,expr c)");
}

TEST(compiler, quasiquote_unquoted_expr__empty)
{
  test_quasiquote_unquoted_expr("#()", "(a b c)");
}

TEST(compiler, quasiquote_unquoted_expr__unquote)
{
  test_quasiquote_unquoted_expr("#(b (+ 1 2))", "(a ,b ,(+ 1 2) c)");
}

TEST(compiler, quasiquote_unquoted_expr__unquote_splicing)
{
...
test_qqtmpl_eq__equal__internal(ScmObj qq1, ScmObj qq2)
{
  ...
  TEST_ASSERT_EQUAL_INT(0, scm_qqtmpl_eq(qq1, qq2, &actual));
                         ...
                         void (*test)(ScmObj, ScmObj))
{
  ...
  q1 = scm_compile_qq_template(t1);
  q2 = scm_compile_qq_template(t2);
...
TEST(compiler, qqtmpl_eq__equal__literal)
{
  test_qqtmpl_eq__equal("(a b c)");
}

TEST(compiler, qqtmpl_eq__not_equal__literal)
{
  test_qqtmpl_eq__not_equal("(a b c)", "(a b 1)");
}

TEST(compiler, qqtmpl_eq__equal__unquote)
{
  test_qqtmpl_eq__equal(",expr");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote)
{
  test_qqtmpl_eq__not_equal(",expr", ",1");
}

TEST(compiler, qqtmpl_eq__equal__unquote__list)
{
  test_qqtmpl_eq__equal("(a ,expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote__list)
{
  test_qqtmpl_eq__not_equal("(a ,expr c)", "(a b ,expr)");
}

TEST(compiler, qqtmpl_eq__equal__unquote__vector)
{
  test_qqtmpl_eq__equal("#(a ,expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote__vector)
{
  test_qqtmpl_eq__not_equal("#(a ,expr c)", "#(a b ,expr)");
}

TEST(compiler, qqtmpl_eq__equal__unquote_splicing__list)
{
  test_qqtmpl_eq__equal("(a ,@expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote_splicing__list)
{
  test_qqtmpl_eq__not_equal("(a ,@expr c)", "(a b ,@expr)");
}

TEST(compiler, qqtmpl_eq__equal__unquote_splicing__vector)
{
  test_qqtmpl_eq__equal("#(a ,@expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote_splicing__vector)
{
  test_qqtmpl_eq__not_equal("#(a ,@expr c)", "#(a b ,@expr)");
}

TEST(compiler, qqtmpl_eq__not_equal__literal_unquote)
{
  test_qqtmpl_eq__not_equal("(a (x y z) ,expr)", "(a ,expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__literal_unquote_splicing)
{
  test_qqtmpl_eq__not_equal("(a (x y z) ,@expr)", "(a ,@expr c)");
}

TEST(compiler, qqtmpl_eq__not_equal__unquote_unquote_splicing)
{
