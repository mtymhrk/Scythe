...
scm_fcd_eq_P(ScmObj obj1, ScmObj obj2)
{
  return (scm_fcd_eq_p(obj1, obj2) ? SCM_TRUE_OBJ : SCM_FALSE_OBJ);
}

int
scm_fcd_eqv(ScmObj obj1, ScmObj obj2, bool *rslt)
{
  bool cmp;

  SCM_REFSTK_INIT_REG(&obj1, &obj2);

  if (scm_fcd_eq_p(obj1, obj2)) {
    cmp = true;
  }
  else if (scm_fcd_number_p(obj1)) {
    if (scm_fcd_number_p(obj2)) {
      int r = scm_fcd_num_eq(obj1, obj2, &cmp);
      if (r < 0) return -1;
    }
    else {
      cmp = false;
  ...
  else if (scm_fcd_symbol_p(obj1)) {
    cmp = scm_fcd_symbol_eq_p(obj1, obj2);
  }
  else if (scm_fcd_char_p(obj1)) {
    int r = scm_fcd_char_eq(obj1, obj2, &cmp);
    if (r < 0) return -1;
  }
  else {
    cmp = false;
...
ScmObj
scm_fcd_eqv_P(ScmObj obj1, ScmObj obj2)
{
  bool cmp;
  int r;

  r = scm_fcd_eqv(obj1, obj2, &cmp);
                     ...
                     ScmObj stack1, ScmObj stack2, int *rslt)
{
  ...
  scm_assert(scm_fcd_nil_p(stack1) || scm_fcd_pair_p(stack1));
  scm_assert(scm_fcd_nil_p(stack2) || scm_fcd_pair_p(stack2));
  ...
  for (lst1 = stack1, cnt1 = 0;
       scm_fcd_pair_p(lst1);
       lst1 = scm_fcd_cdr(lst1), cnt1++) {
    elm1 = scm_fcd_car(lst1);
    if (scm_obj_null_p(elm1)) return -1;

    if (scm_fcd_eq_p(obj1, elm1)) break;
  }
  if (scm_obj_null_p(lst1)) return -1;

  if (!scm_fcd_nil_p(lst1)) {     /* 循環構造がある */
    for (lst2 = stack2, cnt2 = 0;
         cnt2 <= cnt1 && scm_fcd_pair_p(lst2);
         lst2 = scm_fcd_cdr(lst2), cnt2++) {
      elm2 = scm_fcd_car(lst2);
      if (scm_obj_null_p(elm2)) return -1;

      if (scm_fcd_eq_p(obj2, elm2)) break;
    }
...
equal_aux(ScmObj obj1, ScmObj obj2, ScmObj stack1, ScmObj stack2, bool *rslt)
{
  ...
  if (!cmp  && scm_type_info_same_p(scm_obj_type(obj1), scm_obj_type(obj2))) {
    if (scm_fcd_pair_p(obj1) || scm_fcd_vector_p(obj1)) {
      ...
      if (cir == SAME_CIRCULATION) {
        ...
        stack1 = scm_fcd_cons(obj1, stack1);
        if (scm_obj_null_p(stack1)) return -1;

        stack2 = scm_fcd_cons(obj2, stack2);
        if (scm_obj_null_p(stack2)) return -1;
      }
    }

    if (scm_fcd_string_p(obj1)) {
      r = scm_fcd_string_eq(obj1, obj2, &cmp);
      if (r < 0) return -1;
    }
    else if (scm_fcd_bytevector_p(obj1)) {
      cmp = (scm_bytevector_cmp(obj1, obj2) == 0) ? true : false;
    }
    else if (scm_fcd_pair_p(obj1)) {
      elm1 = scm_fcd_car(obj1);
      if (scm_obj_null_p(elm1)) return -1;

      elm2 = scm_fcd_car(obj2);
      if (scm_obj_null_p(elm2)) return -1;
      ...
      if (cmp) {
        elm1 = scm_fcd_cdr(obj1);
        if (scm_obj_null_p(elm1)) return -1;

        elm2 = scm_fcd_cdr(obj2);
        if (scm_obj_null_p(elm2)) return -1;
    ...
    else if (scm_fcd_vector_p(obj1)) {
      size_t len1 = scm_fcd_vector_length(obj1);
      size_t len2 = scm_fcd_vector_length(obj2);
      ...
      if (len1 != len2) {
        ...
        for (size_t i = 0; i < len1; i++) {
          elm1 = scm_fcd_vector_ref(obj1, (size_t)i);
          if (scm_obj_null_p(elm1)) return SCM_OBJ_NULL;

          elm2 = scm_fcd_vector_ref(obj2, (size_t)i);
...
int
scm_fcd_equal(ScmObj obj1, ScmObj obj2, bool *rslt)
{
  return equal_aux(obj1, obj2, SCM_NIL_OBJ, SCM_NIL_OBJ, rslt);
}

ScmObj
scm_fcd_equal_P(ScmObj obj1, ScmObj obj2)
{
  bool cmp;
  int r;

  r = scm_fcd_equal(obj1, obj2, &cmp);
  if (r < 0) return SCM_OBJ_NULL;
