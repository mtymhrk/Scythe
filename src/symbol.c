...
escape_print(scm_char_t chr, ScmEncoding *enc, ScmObj port)
{
  ...
  if (scm_enc_same_char_p(enc, chr.bytes, sizeof(chr), '\a'))
    return scm_write_cstr("\\a", SCM_ENC_SRC, port);
  else if (scm_enc_same_char_p(enc, chr.bytes, sizeof(chr), '\b'))
    return scm_write_cstr("\\b", SCM_ENC_SRC, port);
  else if (scm_enc_same_char_p(enc, chr.bytes, sizeof(chr), '\t'))
    return scm_write_cstr("\\t", SCM_ENC_SRC, port);
  else if (scm_enc_same_char_p(enc, chr.bytes, sizeof(chr), '\n'))
    return scm_write_cstr("\\n", SCM_ENC_SRC, port);
  else if (scm_enc_same_char_p(enc, chr.bytes, sizeof(chr), '\r'))
    return scm_write_cstr("\\c", SCM_ENC_SRC, port);
  else
                                  ...
                                  bool *need_vline_p)
{
  ...
  if (scm_enc_same_char_p(enc, ary[idx].bytes, sizeof(ary[0]), '+')
    ...
    r = scm_write_cchr(ary[idx++], enc, port);
    if (r < 0) return -1;
                               ...
                               bool *need_vline_p)
{
  ...
  *need_vline_p = false;
  ...
  case CHK_IDENT_ADAPT:
    r = scm_write_cchr(ary[0], enc, port);
    if (r < 0) return -1;
    break;
  ...
  for (size_t i = 1; i < len; i++) {
    ...
    switch (type) {
    case CHK_IDENT_ADAPT:
      r = scm_write_cchr(ary[i], enc, port);
      if (r < 0) return -1;
      break;
    case CHK_IDENT_PRINT:
      *need_vline_p = true;
...
scm_symbol_write_ext_rep(ScmObj sym, ScmObj port)
{
  ...
  str = scm_get_output_string(strport);
  if (scm_obj_null_p(str)) return -1;

  r = scm_write_string(str, port, -1, -1);
  if (r < 0) return -1;
...
{
  return scm_symbol_p(obj) ? SCM_TRUE_OBJ : SCM_FALSE_OBJ;
}
...
scm_symbol_new(scm_mem_type_t mtype, ScmObj str)
{
  ...
  sym = scm_alloc_mem(&SCM_SYMBOL_TYPE_INFO, 0, mtype);
...
ScmObj
scm_make_symbol_from_cstr(const char *str, ScmEncoding *enc)
{
  ...
  if (enc != NULL && enc != scm_system_encoding()) {
    s = scm_string_encode(s, scm_system_encoding());
    if (scm_obj_null_p(s)) return SCM_OBJ_NULL;
  }
...
ScmObj
scm_make_symbol_from_bin(const void *data, size_t size, ScmEncoding *enc)
{
  ScmObj s = SCM_OBJ_INIT;

  SCM_REFSTK_INIT_REG(&s);

  s = scm_make_string_from_bin(data, size, enc);
  if (scm_obj_null_p(s)) return SCM_OBJ_NULL;
...
size_t
scm_symbol_bytesize(ScmObj sym)
{
  scm_assert_obj_type(sym, &SCM_SYMBOL_TYPE_INFO);
  return scm_string_bytesize(SCM_SYMBOL_STR(sym));
}
...
{
  return scm_symbol_eq_p(sym1, sym2) ? SCM_TRUE_OBJ : SCM_FALSE_OBJ;
}
...

{
  ...
  for (l = lst; scm_pair_p(l); l = scm_cdr(l)) {
    sym = scm_car(l);
    if (!scm_symbol_p(sym)) {
      scm_error("failed to compare symbols: symbol required, but got", 1, sym);
      return -1;
    }
...
ScmObj
scm_symbol_eq_P_lst(ScmObj lst)
{
  bool cmp;
  int r;
  ...
  r = symbol_cmp_fold(lst, scm_symbol_eq_p, &cmp);
...
ScmObj
scm_string_to_symbol(ScmObj str)
{
  ScmEncoding *enc;
  ...
  enc = scm_string_encoding(str);
  if (enc != scm_system_encoding())
    str = scm_string_encode(str, scm_system_encoding());

  return scm_symtbl_symbol(scm_current_symbol_table(), str);
}

char *
scm_symbol_to_cstr(ScmObj sym, char *cstr, size_t size)
{
  return scm_string_to_cstr(SCM_SYMBOL_STR(sym), cstr, size);
}

int
scm_symbol_eq_cstr(ScmObj sym, const char *str, ScmEncoding *enc, bool *cmp)
{
  ...
  x = scm_make_symbol_from_cstr(str, enc);
  if (scm_obj_null_p(x)) return -1;

  if (cmp != NULL)
    *cmp = scm_symbol_eq_p(sym, x);
                     ...
                     ScmObjPrintHandler handler)
{
  ...
  if (kind == SCM_OBJ_PRINT_DISPLAY) {
    int r = scm_write_string(SCM_SYMBOL_STR(obj), port, -1, -1);
    if (r < 0) return -1;
  }
  else {
...
{
  return scm_string_equal_p(key1, key2);
}
...
scm_symtbl_new(scm_mem_type_t mtype)
{
  ...
  tbl = scm_alloc_mem(&SCM_SYMTBL_TYPE_INFO, 0, mtype);
