...
scm_fcd_string_cv(const ScmObj *chr, size_t n)
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (i == 0) {
      enc = scm_fcd_char_encoding(chr[i]);
      str = scm_fcd_string_new(SCM_MEM_HEAP, NULL, 0, enc);
      if (scm_obj_null_p(str)) return SCM_OBJ_NULL;
    }

    e = scm_fcd_char_encoding(chr[i]);
...
const void *
scm_fcd_string_content(ScmObj str)
{
  scm_assert(scm_fcd_string_p(str));
  return scm_string_content(str);
}
...
scm_fcd_string_set_i(ScmObj str, size_t pos, ScmObj chr)
{
  ...
  if (scm_string_encoding(str) != scm_fcd_char_encoding(chr)) {
...
scm_fcd_string_fill_i(ScmObj str, ScmObj fill, ssize_t start, ssize_t end)
{
  ...
  if (scm_string_encoding(str) != scm_fcd_char_encoding(fill)) {
...
ScmObj
scm_fcd_string_encode(ScmObj str, ScmEncoding *enc)
{
  scm_assert(scm_fcd_string_p(str));
  scm_assert(enc != NULL);
...
scm_char_t *
scm_fcd_string_to_cchr_ary(ScmObj str, size_t pos, ssize_t len, scm_char_t *ary)
{
