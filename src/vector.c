...
string_to_vector_aux(ScmObj str, size_t start, size_t n)
{
  ...
  p = scm_fcd_string_to_cchr_ary(str, start, (ssize_t)n, ary);
  if (p == NULL) return SCM_OBJ_NULL;

  enc = scm_fcd_string_encoding(str);
...
scm_fcd_string_to_vector(ScmObj str, ssize_t start, ssize_t end)
{
  ...
  if (start < 0) start = 0;
  if (end < 0) end = (ssize_t)scm_fcd_string_length(str);
...
const void *
scm_fcd_bytevector_content(ScmObj vec)
{
  scm_assert(scm_fcd_bytevector_p(vec));
  return scm_bytevector_content(vec);
}
