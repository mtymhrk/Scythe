...
(define (push-inst-frame cseq)
  (push-inst (cons-inst 'frame) cseq))
(define (push-inst-cframe cseq)
  (push-inst (cons-inst 'cframe) cseq))
(define (push-inst-eframe cseq)
  (push-inst (cons-inst 'eframe) cseq))
(define (push-inst-ecommit narg cseq)
  (push-inst (cons-inst 'ecommit narg) cseq))
(define (push-inst-epop cseq)
  (push-inst (cons-inst 'epop) cseq))
(define (push-inst-erebind narg cseq)
  (push-inst (cons-inst 'erebind narg) cseq))
...
(define (p2-syntax-handler-call cmpl exp arity tail-p cseq)
  (let ((len (vector-length exp)))
    (if tail-p
        (when (> len 2)
          (push-inst-eframe cseq))
        (if (> len 2)
            (push-inst-frame cseq)
      ...
      (when (> nr-vars 0)
        ...
        (let loop ((idx 0))
          (when (< idx nr-vars)
            ...
            (push-inst-push cseq)
            (loop (+ idx 1))))
        (push-inst-ecommit nr-vars cseq)
        (let loop ((idx 0))
          (when (< idx nr-vars)
      ...
      (when (> nr-vars 0)
        ...
        (let loop ((idx 0))
          (when (< idx nr-vars)
            ...
            (push-inst-push cseq)
            (loop (+ idx 1))))
        (push-inst-erebind nr-vars cseq))
