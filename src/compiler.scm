...
(define (push-inst-cframe typ lbl cseq)
  (push-inst (cons-inst 'cframe (list typ lbl)) cseq))
...
(define (push-inst-jmp typ lbl cseq)
  (push-inst (cons-inst 'jmp (list typ lbl)) cseq))
(define (push-inst-jmpt typ lbl cseq)
  (push-inst (cons-inst 'jmpt (list typ lbl)) cseq))
(define (push-inst-jmpf typ lbl cseq)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (generate-label cmpl)
  (compiler-assign-label-id! cmpl))
...
(define (p2-syntax-handler-call cmpl exp arity tail-p cseq)
  (let ((len (vector-length exp))
        (lbl #f))
    (unless tail-p
      (set! lbl (generate-label cmpl))
      (push-inst-cframe 'label lbl cseq))
    (let loop ((idx 2))
      (when (< idx len)
...
(define (p2-syntax-handler-if cmpl exp arity tail-p cseq)
  (let ((lbl-j (if tail-p #f (generate-label cmpl)))
        (lbl-a (generate-label cmpl)))
    (p2-compile-exp cmpl (vector-ref exp 1) 1 #f cseq)
    (push-inst-jmpf 'label lbl-a cseq)
    (p2-compile-exp cmpl (vector-ref exp 2) arity tail-p cseq)
    (unless tail-p
      (push-inst-jmp 'label lbl-j cseq))
...
(define (p2-syntax-handler-cond cmpl exp arity tail-p cseq)
  (let* ((clauses (vector-ref exp 1))
         ...
         (lbl-j (if tail-p #f (generate-label cmpl))))
    (let loop ((idx 0))
      (when (< idx nr-tests)
    ...
    (let loop ((idx (- nr-clauses 1)) (lbl #f))
      (when (>= idx 0)
        (let* ((cls (vector-ref clauses idx))
          ...
          (cond ((eq? typ '>>)
                 (when lbl (push-inst-jmp 'label lbl cseq))
                ...
                ((eq? typ '=>)
                 (let ((lbl-call #f))
                   ...
                   (push-inst-label (vector-ref lbl-c idx) cseq)
                   (unless tail-p
                     (set! lbl-call (generate-label cmpl))
                     (push-inst-cframe 'label lbl-call cseq))
                   (push-inst-push cseq)
    ...
    (cond ((= nr-tests 0)
          ...
          (else
           (let loop ((idx 1))
             (when (< idx nr-tests)
               (p2-compile-exp cmpl (vector-ref exp idx) 1 #f cseq)
               (push-inst-jmpf 'label lbl-j cseq)
               (loop (+ idx 1))))
    ...
    (cond ((= nr-tests 0)
          ...
          (else
           (let loop ((idx 1))
             (when (< idx nr-tests)
               (p2-compile-exp cmpl (vector-ref exp idx) 1 #f cseq)
               (push-inst-jmpt 'label lbl-j cseq)
               (loop (+ idx 1))))
           (p2-compile-exp cmpl (vector-ref exp nr-tests) 1 tail-p cseq)
           (when (> nr-tests 1)
             (push-inst-label lbl-j cseq)
...
(define (p2-syntax-handler-do cmpl exp arity tail-p cseq)
  (let ((vars (vector-ref exp 1))
    ...
    (let ((nr-vars (vector-length vars))
          (lbl-e (generate-label cmpl))
          (lbl-s (generate-label cmpl)))
      (when (> nr-vars 0)
        (let loop ((idx 0))
          (when (< idx nr-vars)
            (p2-compile-exp cmpl (vector-ref inits idx) 1 #f cseq)
      ...
      (let loop ((idx 0))
        (when (< idx nr-vars)
          ...
          (loop (+ idx 1))))
      (p2-compile-exp cmpl (vector-ref exp 4) 1 #f cseq)
      (push-inst-jmpt 'label lbl-e cseq)
      (unless (null? cmds)
      ...
      (push-inst-jmp 'label lbl-s cseq)
