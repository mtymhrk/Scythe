...
(define (unshift-inst-mvpush cseq)
  (unshift-inst (cons-inst 'mvpush) cseq))
...
(define (decons-formals cmpl formals)
  (let loop ((fm formals)
             (params '())
             (vparam #f))
    (cond
     ...
     ((null? fm)
        (cons (list->vector (reverse params)) vparam))
     (else
      (compile-error cmpl "malformed formals")))))
...
(define (decons-lambda cmpl exp)
  ...
  (let ((formals (decons-formals cmpl (car (cdr exp))))
        (body (cdr (cdr exp))))
    (vector (car formals) (cdr formals) body)))
...
(define (decons-mv-binding-spec cmpl mbs)
  (let loop ((mvbindings mbs)
             (formals '())
             (init '()))
    (if (null? mvbindings)
        (cons (list->vector (reverse formals))
              (list->vector (reverse init)))
        (begin
          ...
          (let ((x (car mvbindings)) (f #f) (i #f))
            ...
            (unless (null? (cdr x))
              (compile-error cmpl "malfromed Mv binding spec"))
            (loop (cdr mvbindings)
                  (cons (decons-formals cmpl f) formals)
                  (cons i init)))))))

(define (decons-let-values cmpl exp)
  (let ((x (cdr exp)))
    (unless (pair? x)
      (compile-error cmpl (format "malformed ~a" (car exp))))
    (let ((y (decons-mv-binding-spec cmpl (car x))))
      (vector (car y) (cdr y) (cdr x)))))

(define (syntax-handler-let-values cmpl exp env
                                   arity tail-p toplevel-p rdepth cseq)
  (let* ((x (decons-let-values cmpl exp))
         (formals (vector-ref x 0))
         (inits (vector-ref x 1))
         (body (vector-ref x 2))
         (nr-vars (do ((i (- (vector-length formals) 1) (- i 1)) (n 0))
                      ((< i 0) n)
                    (set! n
                          (+ n (vector-length (car (vector-ref formals i)))))))
         (new-env (if (> nr-vars 0)
                      (cons-env (let ((v (make-vector nr-vars)))
                                  (let loop ((i 0) (j 0))
                                    (if (>= i nr-vars) v
                                        (let ((x (vector-ref formals j)))
                                          (vector-copy! v i (car x))
                                          (loop (+ i (vector-length (car x)))
                                                (+ j 1))))))
                                #f env)
                      env)))
    (when (and (> nr-vars 0) (not tail-p))
      (unshift-inst-epop cseq))
    (cmpl-closure-body cmpl body nr-vars new-env arity tail-p #f rdepth cseq)
    (when (> nr-vars 0)
      (rdepth-add! rdepth -1)
      (unshift-inst-ecommit nr-vars cseq))
    (let loop ((idx (- (vector-length inits) 1)))
      (when (>= idx 0)
        (let ((i (vector-ref inits idx))
              (f (vector-ref formals idx)))
          (let* ((vars (car f))
                 (n (vector-length vars))
                 (v (cdr f))
                 (a (if v (- n) n)))
            (when (> n 0)
              (unshift-inst-mvpush cseq))
            (unless (= a 1)
              (unshift-inst-mrvc a cseq))
            (compile-exp cmpl i env a #f #f rdepth cseq)))
        (loop (- idx 1))))
    (when (> nr-vars 0)
      (unshift-inst-eframe cseq))))
...
(define (syntax-handler-let*-values cmpl exp env
  ...
  (let* ((x (decons-let-values cmpl exp))
         ...
         (body (vector-ref x 2))
         (new-env #f))
    (let ((len (vector-length formals)))
      (let loop ((env env) (idx 0))
        (if (>= idx len)
            (set! new-env env)
            (let* ((f (vector-ref formals idx))
                   (vars (car f))
                   (v (cdr f)))
              (if (= (vector-length vars) 0)
                  ...
                  (begin
                    (unless tail-p
                      (unshift-inst-epop cseq))
                    (loop (cons-env vars v env) (+ idx 1))))))))
    (cmpl-body cmpl body new-env arity tail-p #f rdepth cseq)
    (let loop ((env new-env) (idx (- (vector-length inits) 1)))
      (when (>= idx 0)
        (let* ((f (vector-ref formals idx))
               (n (vector-length (car f)))
               (a (if (cdr f) (- n) n))
               (i (vector-ref inits idx)))
          (when (> n 0)
            (let ll ((j (- n 1)))
              (when (>= j 0)
                (when (assigned-variable? env j 0)
                  (unshift-inst-box j 0 cseq))
                (ll (- j 1))))
            (unshift-inst-ecommit n cseq)
            (unshift-inst-mvpush cseq)
            (set! env (outer-env env))
            (rdepth-add! rdepth -1))
          (unless (= a 1)
            (unshift-inst-mrvc a cseq))
          (compile-exp cmpl i env a #f #f rdepth cseq)
...
(define (decons-select-module cmpl exp)
  (let ((x (cdr exp)))
...
(define compiler-syntax-let-values
  (make-syntax 'let-values syntax-handler-let-values))
...
(register-syntax '(scheme base) compiler-syntax-let-values #t)
(register-syntax '(scheme base) compiler-syntax-let*-values #t)
