...
scm_subr_func_num_eq_P(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  lst = scm_fcd_cons(argv[1], argv[2]);
  if (scm_obj_null_p(lst)) return -1;

  lst = scm_fcd_cons(argv[0], lst);
  if (scm_obj_null_p(lst)) return -1;
...
scm_subr_func_max(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  lst = scm_fcd_cons(argv[0], argv[1]);
  if (scm_obj_null_p(lst)) return -1;
...
scm_subr_func_floor_div(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  return scm_fcd_return_val(val, sizeof(val)/sizeof(val[0]));
}
...
scm_subr_func_apply(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  if (!scm_fcd_procedure_p(argv[0])) {
    scm_capi_error("apply: procedure required, but got", 1, argv[0]);
    return -1;
  }

  arg = scm_fcd_cons(argv[1], argv[2]);
  if (scm_obj_null_p(arg)) return -1;

  prv = SCM_OBJ_NULL;
  for (itr = arg; scm_fcd_pair_p(itr); itr = nxt) {
    obj = scm_fcd_car(itr);
    nxt = scm_fcd_cdr(itr);

    if (scm_fcd_pair_p(nxt)) {
      cur = scm_fcd_cons(obj, SCM_NIL_OBJ);
  ...
  return scm_fcd_trampolining(argv[0], arg, SCM_OBJ_NULL, SCM_OBJ_NULL);
}
...
scm_subr_func_write_simple(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  val = scm_api_write_simple(argv[0], port);
  if (scm_obj_null_p(val)) return -1;
...
scm_subr_func_eval_asm(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  return scm_fcd_trampolining(code, args, SCM_OBJ_NULL, SCM_OBJ_NULL);
}
...
scm_subr_func_eval(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  postproc = scm_fcd_make_subrutine(scm_subr_func_eval__post_compile,
                                    1, 0, subr_mod);
  if (scm_obj_null_p(postproc)) return -1;

  if (scm_fcd_nil_p(argv[1])) {
    cmpl = scm_fcd_make_compiler(SCM_OBJ_NULL);
    if (scm_obj_null_p(cmpl)) return -1;

    args = scm_fcd_list(2, argv[0], cmpl);
  }
  else {
    args = scm_fcd_cons(argv[0], argv[1]);
  }
...
scm_subr_func_module_export(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  val = scm_api_module_export(argv[0], argv[1]);
  if (scm_obj_null_p(val)) return -1;
...
int
scm_subr_func_macro_P(ScmObj subr, int argc, const ScmObj *argv)
{
  ScmObj val = SCM_OBJ_INIT;
...
int
scm_subr_func_make_macro(ScmObj subr, int argc, const ScmObj *argv)
{
  ScmObj val = SCM_OBJ_INIT;
  ...
  val = scm_api_make_macro(argv[0], argv[1]);
  if (scm_obj_null_p(val)) return -1;
...
int
scm_subr_func_macro_env(ScmObj subr, int argc, const ScmObj *argv)
{
  ScmObj val = SCM_OBJ_INIT;
...
scm_subr_func_eval_file__loop(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  if (scm_fcd_eof_object_p(exp))
    return scm_fcd_return_val(argv + 1, argc - 1);
...
scm_subr_func_eval_file(ScmObj subr, int argc, const ScmObj *argv)
{
  ...
  loop = scm_fcd_make_subrutine(scm_subr_func_eval_file__loop,
